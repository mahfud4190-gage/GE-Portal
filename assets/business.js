...
function pmPageKeyR2(){return (window.GE_ROUTE||location.pathname.split('/').pop()||'index.html')}
...
function geV2532FixActiveNav(){
  const page=(window.GE_ROUTE||location.pathname.split('/').pop()||'index.html');
  document.querySelectorAll('.side a[href]').forEach(a=>{
    const href=(a.getAttribute('href')||'').split('?')[0];
...
function applyPlanningActions(){
 const page=(window.GE_ROUTE||location.pathname.split('/').pop()||'index.html').toLowerCase();
