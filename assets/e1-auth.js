...
function gxGetPortalData(){return null}
function gxGetUsers(){return []}
function gxGetSession(){try{return JSON.parse(sessionStorage.getItem(GX_SESSION_KEY)||'null')}catch(e){return null}}
function gxSetSession(session){if(!session)return;try{sessionStorage.setItem(GX_SESSION_KEY,JSON.stringify(session))}catch(e){}window.GX_CURRENT_USER=session}
function gxClearSession(){try{sessionStorage.removeItem(GX_SESSION_KEY);sessionStorage.removeItem(GX_RETURN_TO_KEY);sessionStorage.removeItem(GX_SYNC_KEY)}catch(e){}window.GX_CURRENT_USER=null}
...
function gxRememberReturnTo(route=gxCurrentRoute()){return gxSafeReturnTo(route)}
function gxConsumeReturnTo(){let candidate='';try{candidate=new URLSearchParams(location.search).get('next')||sessionStorage.getItem(GX_RETURN_TO_KEY)||'';sessionStorage.removeItem(GX_RETURN_TO_KEY)}catch(e){}return gxSafeReturnTo(candidate)}
function gxAuditDirect(action,module,objectLabel,detail,userOverride=null){window.dispatchEvent(new CustomEvent('gx-audit-event',{detail:{action,module,object:objectLabel||'',detail:detail||'',user:userOverride||gxGetSession()||{}}}))}
