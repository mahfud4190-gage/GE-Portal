...
(function(){
  ...
  function ensureModal(id,html){let m=document.getElementById(id);if(!m){m=document.createElement('div');m.id=id;m.className='modal-backdrop ge-modal-root';m.setAttribute('role','presentation');m.innerHTML=html;document.body.appendChild(m)}return m}
  function showModal(m){m.classList.add('show');document.body.classList.add('ge-modal-open')}
  function hideModal(m){m.classList.remove('show');if(!document.querySelector('.modal-backdrop.show'))document.body.classList.remove('ge-modal-open')}
  ...
  function openUserForm(id=null){
    ...
    modal.querySelector('#p26FormHost').innerHTML=formMarkup(u);showModal(modal);
  ...
  function closeUserForm(){const m=document.getElementById('p26UserModal');if(m)hideModal(m)}
  ...
  function openImportModal(){
    ...
    showModal(modal);const close=()=>hideModal(modal);modal.querySelector('#p26ImportClose').onclick=close;modal.querySelector('#p26ImportCancel').onclick=close;modal.querySelector('#p26ImportFile').onchange=()=>previewImport(modal,modal.querySelector('#p26ImportFile').files[0]);modal.querySelector('#p26ImportConfirm').onclick=()=>confirmImport(modal);
  }
