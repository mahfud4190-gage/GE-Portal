...
async function actorFor(event){
  ...
  const actor={id:decoded.uid,...snap.data()}; const rn=String(actor.role||'').replace(/[\s_-]+/g,'').toLowerCase(); if(rn==='superadmin')actor.role='Super Admin'; else if(rn==='headoffice')actor.role='Head Office'; else if(rn==='branchoffice')actor.role='Branch Office'; if(!Array.isArray(actor.airports)&&Array.isArray(actor.assignedStations))actor.airports=actor.assignedStations;
...
async function readCollection(db,actor,c){
  ...
  if(c==='initiatives' && EXTERNAL_ROLES.has(actor.role)){
    const [a,b]=await Promise.all([
      db.collection('portalData').doc(c).collection('records').where('sharedWithUserIds','array-contains',String(actor.id)).get(),
      db.collection('portalData').doc(c).collection('records').where('mentionedUserIds','array-contains',String(actor.id)).get()
    ]); const map=new Map(); [...a.docs,...b.docs].forEach(d=>map.set(d.id,d)); docs=[...map.values()];
  } else if(c==='inbox' && !ADMIN_ROLES.has(actor.role)){
    const snap=await db.collection('portalData').doc(c).collection('records').where('recipientId','==',String(actor.id)).get(); docs=snap.docs;
  } else {
    const ref=(c==='users')?db.collection('users'):db.collection('portalData').doc(c).collection('records'); const snap=await ref.get(); docs=snap.docs;
  }
...
async function writeOne(db,actor,collection,action,id,raw){
  ...
  const base=(collection==='users')?db.collection('users'):db.collection('portalData').doc(collection).collection('records'); const ref=id?base.doc(cleanId(id)):base.doc();
  if(action==='DELETE'){
    ...
    await ref.delete();
    await db.collection('portalData').doc('auditLogs').collection('records').add({actorId:actor.id,actorRole:actor.role,name:actor.name||actor.username||actor.email||actor.id,username:actor.username||actor.email||'',module:collection,object:ref.id,detail:`${collection} ${action.toLowerCase()}`,targetType:`EDITION1_${collection.toUpperCase()}`,targetId:ref.id,action:'Delete',timestamp:FieldValue.serverTimestamp(),result:'SUCCESS'});
